...
import { useBrainstormCanvasModel } from './useBrainstormCanvasModel';
...
export function BrainstormCanvas({
  ...
  const {
    ...
    setZoom,
    canvasRef,
  } = useBrainstormCanvasModel(initialCanvas, seeds, drafts);
  ...
  const updateEdgeLabel = (id: string, label: string) => {
    const newEdges = canvas.edges.map((e) =>
      e.id === id ? { ...e, label } : e
    );
    const newCanvas = { ...canvas, edges: newEdges };
    setCanvas(newCanvas);
  ...
  const handleExport = async () => {
    ...
    const report =
      `# Brainstorm Topology Export\n\nGenerated on ${new Date().toLocaleString()}\n\n\`\`\`mermaid\n${mermaid}\`\`\`\n\n## Node Details\n\n` +
      canvas.nodes
        .map((n) => `### ${n.title} (${n.nodeType})\n${n.content}`)
        .join('\n\n');
    ...
    try {
      ...
      showInfo(
        'Topology exported to clipboard and saved to .kspec/brainstorm/topology.md'
      );
    } catch (err) {
  ...
  const handleNodeDrag = (id: string, info: PanInfo) => {
    ...
    const newNodes = canvas.nodes.map((node) => {
      if (
        node.id === id ||
        (isPartOfSelection && selectedNodeIds.includes(node.id))
      ) {
        return {
          ...node,
          x: node.x + deltaX,
  ...
  const deleteNode = (id: string) => {
    ...
    const newEdges = canvas.edges.filter(
      (e) =>
        !idsToDelete.includes(e.fromNode) && !idsToDelete.includes(e.toNode)
    );
    ...
    <div className="flex flex-col h-[500px] bg-black/20 rounded-2xl border border-white/5 overflow-hidden relative">
              ...
              return (
                <g key={edge.id} className="group/edge cursor-pointer">
                  ...
                  <line
                    ...
                    strokeDasharray={
                      selectedEdgeId === edge.id ? 'none' : '4 4'
                    }
            ...
            {canvas.nodes.map((node) => (
              <motion.div
                ...
                className={cn(
                  ...
                  selectedNodeIds.includes(node.id) &&
                    'brainstorm-node-selected border-white/40 ring-4 ring-white/5',
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className="brainstorm-node-type">{node.nodeType}</span>
              ...
              {selectedNode ? (
                <>
                  <div className="space-y-3">
                    <div>
                      <label className="text-[9px] font-bold text-white/20 uppercase tracking-widest mb-1 block">
                        Title
                      </label>
                      <input
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    >
                      <ShieldCheck
                        size={14}
                        className={cn(
                          isProcessing === 'review' && 'animate-spin'
                        )}
                      />
                      Architect Review
                    </button>
                    ...
                    >
                      <PenTool
                        size={14}
                        className={cn(
                          isProcessing === 'sketch' && 'animate-spin'
                        )}
                      />
                      Agentic Sketch
                    </button>
                      ...
                      <div className="space-y-2 pt-2 border-t border-white/5 mt-2">
                        ...
                        >
                          <FileEdit
                            size={14}
                            className={cn(
                              isProcessing === 'draft' && 'animate-spin'
                            )}
                          />
                          Promote to Draft
                        </button>
                        ...
                        >
                          <CheckCircle2
                            size={14}
                            className={cn(
                              isProcessing === 'task' && 'animate-spin'
                            )}
                          />
                          Convert to Task
                        </button>
                    ...
                    {selectedNode.nodeType === 'seed' && (
                      <div className="p-3 rounded-xl bg-white/5 border border-white/5 text-[9px] text-white/30 italic">
                        This is a raw seed. Run Architect Review to refine the
                        technical approach.
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5 text-[10px] text-white/40 leading-relaxed italic">
                    {selectedNodeIds.length} nodes selected. You can move them
                    together or perform bulk operations.
                  </div>
      ...
      {canvas.nodes.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
          ...
          <h3 className="text-xs font-bold text-white/40 mb-2">
            Visual Workspace Empty
          </h3>
          <p className="text-[10px] text-white/20 leading-relaxed max-w-[240px] mb-6">
            Start mind-mapping by adding seeds or drafts from the inventory
            toolbar above.
          </p>
          <div className="grid grid-cols-2 gap-3 w-full max-w-[300px]">
            <div className="p-3 rounded-xl bg-indigo-500/5 border border-indigo-500/10 text-left">
              ...
              <div className="text-[9px] font-bold text-indigo-300/80 mb-1">
                Seeds
              </div>
              <div className="text-[9px] text-white/20">
                Raw ideas waiting for architectural refinement.
              </div>
            </div>
            <div className="p-3 rounded-xl bg-purple-500/5 border border-purple-500/10 text-left">
              ...
              <div className="text-[9px] font-bold text-purple-300/80 mb-1">
                Drafts
              </div>
              <div className="text-[9px] text-white/20">
                KSpec modules being drafted for implementation.
              </div>
            </div>
          </div>
        </div>
