...
describe('OrchestrationPanel runtime HUD', () => {
  it('streams telemetry and agent status into functional HUD elements', () => {
    ...
    expect(screen.getByTitle('Agent Tokens')).toHaveTextContent('0');
    expect(screen.getByTitle('Agent Cost Estimate')).toHaveTextContent(
      '$0.0000'
    );
  ...
  it('adds activity feed items, file metadata, and approval popups from agent actions', () => {
    ...
    expect(screen.getByTitle('Agent Tokens')).toHaveTextContent('34');
    expect(screen.getByTitle('Agent Cost Estimate')).toHaveTextContent(
      '$0.0120'
    );
  });
  ...
  it('approves and rejects popup approvals through the API and removes them from screen', async () => {
    ...
    await waitFor(() =>
      expect(approveAction).toHaveBeenCalledWith('approval-1')
    );
    const approvalsSection = screen.getByText('Pending Approvals').parentElement;
    if (!approvalsSection) throw new Error('Pending approvals section missing');
    expect(within(approvalsSection).queryByText('Approve optimization?')).not.toBeInTheDocument();

    fireEvent.click(screen.getAllByText('Reject')[0]);
    await waitFor(() =>
      expect(rejectAction).toHaveBeenCalledWith('approval-2')
    );
